...
<body style="font-family: Eurostar">
    ...
    <div style="width: 40%; float: left; text-align: right; padding-top: 20px; ">
        No. : <?= $model[0]['supplierPaymentNum'] ?> <br/>
        No. Voucher : <?= $model[0]['voucherNum'] ?> <br/>
        <?php if ($model[0]['supplierPaymentDate']) echo "Tgl : ".date('d-M-Y', strtotime($model[0]['supplierPaymentDate'])); ?>
    </div>
    ...
    <table class="table-report" style="width: 100%; border: 2px solid black; margin-top: 20px; margin-bottom: 10px; border-collapse: collapse;">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td style="border-top: 1px solid black; width: 25%; text-align: right;"><?= number_format($modelDetail['paymentAmount'],2,",",".") ?></td>
            </tr>
            <?php 
                $totalPrice = $totalPrice + $modelDetail['paymentAmount'];
                $no = $no + 1;
            } ?>
            <tr>
