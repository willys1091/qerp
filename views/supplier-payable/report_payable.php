...
$supplier = MsSupplier::findOne($supplierID);
?>
<!DOCTYPE html>
<html>
<head>
...
<body style="font-family: Eurostar">
    <div id='header'>
        <div style="text-align: center"><?= Html::img('assets_b/images/image.jpg',['height' => '85px', 'width' => '80px']) ?> </div>
        <br/> 
        <div style="font-size: 25px; color: green; text-align: center; margin-top: -20px;"><?= AppHelper::getSetting('CompanyName','Company Name') ?></div>
        <p><b>Supplier : <?=$supplier['supplierName']?></b></p>
    </div>
    <div class='row'>
        <table style="width:100%; border-collapse: collapse;">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class='text-right'><?= $detail['currency'].' '.number_format($detail['grandTotal'], 2, ',', '.') ?></td>
                    <td class='text-left'><?=$detail['currency'].' '
                        .number_format(floatVal($detail['advancedPaymentAmount'])
                            + floatVal($detail['previousPayment']), 2, ',', '.');?></td>
                    <td class='text-right'><?= $detail['currency'].' '.number_format($detail['outstandingAmount'], 2, ',', '.'); ?></td>
                <?php $no++; }?>
            </tbody>
