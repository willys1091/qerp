    ...
    <div class="box box-primary box-solid">
        ...
        <div class="box-body">
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    <div class="row">
                        ...
                        <div class="col-md-4">
                            <?= $form->field($model, 'goodsReceiptDate')->widget(DatePicker::className(), AppHelper::getDatePickerConfig(['disabled' => isset($isApprove), 'disabled' => true])) ?>
                        </div>
                    </div>
                    <?= Html::activeHiddenInput($model, 'goodsReceiptDateHidden', ['class' => '']) ?>
                ...
                <div class="panel-body">
                    ...
                    <div class="row">
                        <div class="col-md-4 pibSubmitCode">
                            <?= $form->field($model, 'pibSubmitCode')->textInput(['maxlength' => true]) ?>
                        </div>
                        <div class="col-md-4 pibRate">
                    ...
                    <div class="row">
                        <div class="col-md-4">
                            ...
                            ->widget(\yii\widgets\MaskedInput::classname(), [
                                    ...
                                    'options' => [
                                        'class' => 'form-control text-right cif isImport'
                                    ],
                                    ])
                            ->label(false)
                            ?>
                        ...
                        <div class="col-md-4">
                            ...
                            ->widget(\yii\widgets\MaskedInput::classname(), [
                                    ...
                                    'options' => [
                                        'class' => 'form-control text-right fob isImport'
                                    ],
                                    ])
                            ->label(false)
                            ?>
            ...
            <div class="panel panel-default">
                <div class="panel-heading">
                    <div style="display: inline-block; margin: 7px 0px;">Transaction Summary</div>
                    <?php if (!isset($isView)): ?>
                    <button type="button" class="btn btn-primary btnCalculate" title="Calculate transaction summary" style="right: 20px; position: absolute;"><span class="fa fa-calculator"></span> Calculate</button>
                    <?php endif; ?>
                </div>
                <div class="panel-body">
                    <div class="row">
                        <div class="col-md-6">
                            <label class="control-label isImport">Import Duty (IDR)</label>
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <label class="control-label isImport">PPN Import (IDR)</label>
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <label class="control-label isImport">PPH Pasal 22 Import (IDR)</label>
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <label class="control-label text-right">Total Import Tax (IDR)</label>
                            <?= MaskedInput::widget([
                                'name' =>  'totalImportTax',
                                'clientOptions' => [
                                    'alias' => 'decimal',
                                ...
                                'options' => [
                                    'class' => 'form-control text-right totalImportTax',
                                    'readonly' => 'readonly'
                                ],
                            ])
                            ?>
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <label class="control-label isImport">Nilai Faktur Pajak</label>
                            <?= $form->field($model, 'taxInvoiceAmount')
                            ->widget(\yii\widgets\MaskedInput::classname(), [
                                    'clientOptions' => [
                                    'alias' => 'decimal',
                                    'digits' => 2,
                                    ...
                                    'removeMaskOnSubmit' => false
                                        ...
                                        'options' => [
                                            'class' => 'form-control text-right  isImport'
                                        ],
                                    ])
                            ->label(false)
                            ?>
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <label class="control-label isImport">Biaya Lain - Lain</label>
                            <?= $form->field($model, 'otherCostAmount')
                            ->widget(\yii\widgets\MaskedInput::classname(), [
                                    'clientOptions' => [
                                    'alias' => 'decimal',
                                    'digits' => 2,
        ...
        <div class="box-footer">
            <div class="form-group text-right">
                <?php if (!isset($isView)): ?>
                    <?= Html::submitButton('<i class="glyphicon glyphicon-save"> Submit </i>', ['class' => 'btn btn-primary btnSave']) ?>
                <?php endif; ?>
                
                <?php if (!isset($isView)){ ?>
                    <?= AppHelper::getCancelButton() ?>
                <?php } else { ?>
                    <?= Html::a('<i class="glyphicon glyphicon-remove"> Back </i>', ['index'], ['class'=>'btn btn-danger']) ?>
                <?php } ?>
            </div>
        </div>
    </div>
...
$importDutyAmount = $model->importDutyAmount;
$PPNImportAmount = $model->PPNImportAmount;
$PPHImportAmount = $model->PPHImportAmount;
ClientScript::singleton()->beginScript('js');
?>
<script>    
    var rate = <?=$rate?>;
    var initValue = <?=$goodsDetail?>;
    var importDutyAmount = <?=$importDutyAmount?>;
    var PPNImportAmount = <?=$PPNImportAmount?>;
    var PPHImportAmount = <?=$PPHImportAmount?>;
    console.log(importDutyAmount);
    var rowTemplate = "" +
        "<tr>" +
        ...
        "   </td>" +
        "   <td>" +
        "       <input type='text' class='form-control text-center goodsDetailHSCodeTax' name='TrGoodsreceiptheadapproval[joinGoodsReceiptDetail][{{Count}}][hsCodeTax]' value='{{hsCodeTax}}' >" +
        "   </td>" +
        "</tr>";
    ...
    function addRow(detailID, productID, productName, uomID, uomName, qty, packID, packName, packQty, price, discount, taxValue, tax, subTotal, condition, hsCodeTax){
        ...
        template = replaceAll(template, '{{price}}', price.formatMoney());
        template = replaceAll(template, '{{discount}}', discount.formatMoney());
        template = replaceAll(template, '{{subTotal}}', subTotal.formatMoney());
    ...
    {
        var pibRate = $('#trgoodsreceiptheadapproval-pibrate').val();
        if (pibRate)
        {
            pibRate = pibRate.currencyToFloat();
        } else
        {
            bootbox.alert('Please enter PIB Rate first');
            return;
        }
        ...
        var hsCodeTaxTotal = 0;
        var PPNImportTotal = 0;
        var PPHImportTotal = 0;
        ...
        $('.goods-detail-table tbody').each(function() {
            $('tr', this).each(function () {
                ...
                discTotal += qtyValue * priceValue * (discValue / 100);
                subTotal += (qtyValue * priceValue);
                
                hsCodeTaxTotal += qtyValue * priceValue * ((100-discValue)/100) * (hsCodeTaxValue / 100);
                //PPNImportTotal += ((qtyValue * priceValue * ((100-discValue)/100)) + hsCodeTaxTotal) * (<?=MsSetting::getSetting('ImportTax')?> / 100) * pibRate;
                PPHImportTotal += ((qtyValue * priceValue * ((100-discValue)/100)) + hsCodeTaxTotal) * (2.5 / 100) * pibRate;
            })
        });
        hsCodeTaxTotal *= pibRate;
      
        if(importDutyAmount == 0 && PPNImportAmount == 0 && PPHImportAmount == 0)
            taxImportTotal = hsCodeTaxTotal + PPNImportTotal + PPHImportTotal;
        else
            taxImportTotal = hsCodeTaxTotal + PPNImportTotal + PPHImportTotal;
        ...
        $('.subTotalSummary').val(subTotal.formatMoney());
        $('.discTotalSummary').val(discTotal.formatMoney());
        $('.taxTotalSummary').val(taxTotal.formatMoney());
        $('.grandTotalSummary').val(grandTotal.formatMoney());
        $('.outstandingSummary').val(outstandingTotal.formatMoney());
        $('.PPHImport').val(PPHImportTotal.formatMoney());
        if(!<?=$isUpdate?>){
        
            if(importDutyAmount == 0)
                $('.importDuty').val(hsCodeTaxTotal.formatMoney());
        }
       
        if(PPNImportAmount == 0)
            $('.PPNImport').val(PPNImportTotal.formatMoney());
        if(PPHImportAmount == 0)
            $('.PPHImport').val(PPHImportTotal.formatMoney());
        $('.totalImportTax').val(taxImportTotal.formatMoney());
    }
    ...
    if($('#trgoodsreceiptheadapproval-pibrate').val())
    {
        var total = importDutyAmount + PPNImportAmount + PPHImportAmount;
        var subTotal = 0;
      
        
        $('.totalImportTax').val(total.formatMoney());
...
<?php
ClientScript::singleton()->endScript();
?>