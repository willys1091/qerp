...
class XLSXWriter {
    ...
    public function writeSheetHeader($sheet_name, array $header_types, $col_options = null) {
        ...
        if (!$suppress_row) {
            ...
            $sheet->file_writer->write('<row ht="12.1" outlineLevel="0" r="' . (1) . '">');
    ...
    public function writeSheetRow($sheet_name, array $row, $row_options = null) {
        ...
        if (!empty($row_options)) {
            ...
            $sheet->file_writer->write('<row ht="' . ($ht) . '" outlineLevel="0" r="' . ($sheet->row_count + 1) . '">');
        } else {
            $sheet->file_writer->write('<row ht="12.1" outlineLevel="0" r="' . ($sheet->row_count + 1) . '">');
        }
    ...
    public function writeSheetRowInString($sheet_name, array $row, $row_options = null) {
        if (empty($sheet_name))
            return;
        self::initializeSheet($sheet_name);
        $sheet = &$this->sheets[$sheet_name];
